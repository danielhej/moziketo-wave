...
async def start_play(body: DownloaderPlayRequest) -> DownloaderPlayResponse:
    settings = get_settings()
    if not downloader_configured():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
    ...
    return DownloaderPlayResponse.model_validate(response.json())


async def get_play_job(job_id: str) -> DownloaderJobStatusResponse:
    settings = get_settings()
    if not downloader_configured():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(15.0, connect=10.0)) as client:
            response = await client.get(
                f"{settings.downloader_url.rstrip('/')}/v1/jobs/{job_id}",
                headers=headers,
            )
    except httpx.RequestError as exc:
        raise HTTPException(
    ...
    return DownloaderJobStatusResponse.model_validate(response.json())
